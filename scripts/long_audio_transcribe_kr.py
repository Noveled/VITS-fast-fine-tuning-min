...
if __name__ == "__main__":
    ...
    # Whisper 모델 로드 (명령줄 인자로 받은 whisper 모델 크기를 사용)
    model = whisper.load_model(args.whisper_size)

    # 화자 정보(오디오 파일 경로, 화자 이름, 텍스트)를 저장할 리스트
    speaker_annos = []

    # filelist 내의 각 오디오 파일에 대해 처리
    for file in filelist:
        print(f"transcribing {parent_dir + file}...\n")  # 파일 처리 시작 메시지 출력

        # Whisper 모델을 사용할 때의 설정 (빔 사이즈와 결과 선택 옵션 설정)
        ...
        # 지원하지 않는 언어인 경우 무시하고 다음 파일로 넘어감
        ...
        for i, seg in enumerate(result['segments']):
            ...
            # 세그먼트 파일명 생성 (캐릭터 이름, 코드, 세그먼트 번호 사용)
    ...
    # speaker_annos 리스트에 저장된 화자 정보를 long_character_anno.txt 파일에 저장
