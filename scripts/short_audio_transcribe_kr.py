...
# 언어별 토큰을 정의하는 딕셔너리, 중국어, 일본어, 영어를 나타냅니다.
lang2token = {
            'zh': "[ZH]",
            'ja': "[JA]",
            "en": "[EN]",
...
def transcribe_one(audio_path):
    ...
    # 오디오 파일을 로드하고, 30초로 패드하거나 자릅니다.
    audio = whisper.load_audio(audio_path)
    audio = whisper.pad_or_trim(audio)

    # make log-Mel spectrogram and move to the same device as the model
    # 로그-멜 스펙트로그램을 생성하고, 모델이 사용하는 디바이스(CPU 또는 GPU)로 이동시킵니다.
...
if __name__ == "__main__":
    ...
    # GPU가 사용 가능한지 확인, 사용 불가능하면 에러 발생
    assert (torch.cuda.is_available()), "Please enable GPU in order to run Whisper!"

    # Whisper 모델을 로드합니다. 인자로 받은 모델 크기를 사용.
    ...
    for speaker in speaker_names:
        ...
        for i, wavfile in enumerate(list(os.walk(parent_dir + speaker))[0][2]):
            ...
            # 파일 이름이 "processed_"로 시작하면 건너뜁니다.
            ...
            try:
                ...
                # 샘플링 레이트가 타겟 샘플링 레이트와 다르면 다시 샘플링합니다.
                if sr != target_sr:
